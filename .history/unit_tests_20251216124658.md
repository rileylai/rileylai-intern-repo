...
This section describes what I learned about testing Redux reducers and actions using Jest.

---

## 1. What was the most challenging part of testing Redux?

When testing Redux, I found the following parts challenging:

1. Understanding how reducers update state based on actions.
2. Writing tests for pure functions instead of UI behavior.
3. Setting up initial state correctly in tests.
4. Testing asynchronous actions (such as thunks).
5. Knowing what should be tested in Redux and what belongs to component tests.

After practice, testing reducers became more straightforward.

---

## 2. How do Redux tests differ from React component tests?

Redux tests are different from React component tests because:

1. Redux tests focus on state changes, not UI rendering.
2. Reducers are pure functions and easier to test.
3. No DOM or user interaction is involved.
4. Tests are faster and more isolated.
5. Redux tests help ensure business logic works correctly.

This separation makes the overall test structure cleaner.

---

## Summary (Redux Testing)

Testing Redux with Jest helps ensure state management logic works as expected.  
Although Redux testing felt abstract at first, it improved my understanding of application data flow.

---

## Task 76 - Evidence of Implementation

I made a new folder `tasks/task-076-mocking-api-jest/`. It holds `api.js`, `App.jsx`, and `App.test.js` where the React component, API module, and Jest test are implemented. Screenshots will be in `tasks/task-076-mocking-api-jest/screenshots/`. Run the test with `npm test`.
